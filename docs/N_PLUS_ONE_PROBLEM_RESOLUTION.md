...
현재 통합 테스트는 10개, 100개 정도의 주문으로 진행했다. Hibernate Statistics로 쿼리 수가 2개로 줄어든 걸 확인했고, 98% 성능 개선이라는 수치를 얻었다.

하지만 이건 개발 환경에서의 결과일 뿐이다. 실제 운영 환경과는 차이가 있을 수 있다.

**실제 운영 환경에서는:**
- 데이터베이스가 원격에 있어서 네트워크 레이턴시가 있다 (5~10ms)
- 수만 건의 주문이 쌓여 있다
- 동시에 여러 요청이 들어온다

이런 환경에서 진짜 성능 차이를 측정하려면:
- 실제 운영 수준의 데이터 (최소 수십만 건)
- 프로덕션 유사 환경 (AWS RDS 등)
- 실제 트래픽 패턴으로 부하 테스트

다음 단계로는 더 현실적인 환경에서 성능을 검증해봐야 할 것 같다.
