...
Saga 패턴(Orchestration/Choreography)에 따라 실패 처리 방식이 다르다.

- **Orchestration**: 중앙 오케스트레이터가 상태 관리, 중단 지점부터 재시작 가능
- **Choreography**: 각 서비스가 독립적으로 DLT 관리, 보상 이벤트로 롤백

> 자세한 내용은 [MSA 트랜잭션 설계 문서](./10_MSA_TRANSACTION_DESIGN_REPORT.md) 참고
...
```
Redis: 동시성 제어, 수량 체크 (INCR 또는 Set + List)
Kafka: 비동기 후처리 (DB 저장 등 무거운 작업)
```

Redis 대기열(List)을 Kafka로 전환하면:
- 스케줄러 폴링 → 이벤트 기반 처리
- 컨슈머 스케일 아웃으로 처리량 증가
- 메시지 영속화로 유실 방지
- MSA 전환 시 서비스 분리 용이
...
Redis에서 race condition으로 101명이 통과해도, 컨슈머가 DB 카운트 체크로 100명만 처리한다. 분산 환경에서 최종 검증은 필수다.

> Redis 구현 상세(INCR vs Set, Set + List 구조)는 [Redis 시스템 설계 문서](./09_REDIS_SYSTEM_DESIGN_REPORT.md) 참고
